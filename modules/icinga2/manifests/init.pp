...
class icinga2_ido_pgsql (
  ...
  exec { 'create-pgsql-icinga2-ido-db':
    ...
    command => "sudo -u postgres psql -c \"CREATE ROLE ${ido_db_user} WITH LOGIN PASSWORD \'${ido_db_pass}\';\" && sudo -u postgres createdb -O ${ido_db_name} -E UTF8 ${ido_db_name} && sudo -u postgres createlang plpgsql ${ido_db_name}",
...
# Defined Type to turn on Various Icinga2 Features
#
